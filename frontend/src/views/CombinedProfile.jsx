...
const INSTINCT_META = {
  ...
  sx: { label: 'Sexual / One-to-One' },
  so: { label: 'Social' },
};

// Your integrated profile once all three assessments are saved. Rendered by
// GuidedTyper in its 'combined' phase; reads results straight from localStorage.
export default function CombinedProfile({ onBack = () => {} }) {
  ...
  if (doneCount === 0) {
    return (
      <div style={{ ...S.cardGold, textAlign: 'center', padding: '32px 24px' }}>
        ...
        <button onClick={() => onBack()} style={S.btn}>Start Assessments →</button>
      </div>
    );
  }
  ...
  if (doneCount < 3) {
    ...
    return (
      <div style={S.card}>
        ...
        <button onClick={() => onBack()} style={S.btn}>Continue Assessments →</button>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <button onClick={() => onBack()} style={{ ...S.btnOutline, width: '100%', marginTop: 8 }}>
        ← Back to My Tests
      </button>
    </>
  );
