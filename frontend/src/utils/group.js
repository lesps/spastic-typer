...
export function analyzeGroup(persons) {
  ...
  if (ep.length >= 2) {
    ...
    if (activeCenters.length === 1) {
      ...
      ennInsights.push({ icon: '◆', label: `Single Center: All ${ctr.charAt(0).toUpperCase() + ctr.slice(1)}`, desc: `All members operate from the ${ctr} center, focused on ${concern}. No ${missing} perspective is represented — the group may have collective blind spots around those domains.`, color: '#e88050' });
    } else if (activeCenters.length === 3) {
      ennInsights.push({ icon: '◆', label: 'Balanced Center Spread', desc: 'Gut, heart, and head centers are all represented. The group has access to the full range of instinctual perspectives: action and autonomy, identity and connection, and analysis and planning. The challenge is translation — each center speaks a different emotional language.', color: '#50c878' });
    } else {
      ...
      if (dom[1] > sorted[1][1]) {
        ...
        ennInsights.push({ icon: '◆', label: `${dom[0].charAt(0).toUpperCase() + dom[0].slice(1)}-Dominant Group`, desc: `The ${dom[0]} center is overrepresented, with ${others} less present. The group's emotional vocabulary skews toward ${dom[0] === 'gut' ? 'action and autonomy' : dom[0] === 'heart' ? 'identity and connection' : 'analysis and planning'}.`, color: G.gold });
      }
    }
    ...
    if (activeH.length === 1) {
      ...
      ennInsights.push({ icon: '⟷', label: `Uniform Conflict Style: ${h.charAt(0).toUpperCase() + h.slice(1)}`, desc: `All members use the ${h} harmonic strategy — they all tend to ${hDesc} when stressed. Uniform conflict style reduces friction but creates shared blind spots.`, color: '#e88050' });
    } else if (activeH.length === 3) {
      ennInsights.push({ icon: '⟷', label: 'All Three Conflict Styles Present', desc: 'Competency, Reactive, and Positive harmonic groups are all represented. The group will handle stress in very different ways — expect friction around conflict style before the actual content of conflicts.', color: '#4a88d8' });
    }

    const stressLinks = [];
    ep.forEach((pA, i) => ep.forEach((pB, j) => { if (i !== j && ENN_ARROWS[pA.ennType].stress === pB.ennType) stressLinks.push(`${pA.label} \u2192 ${pB.label}`); }));
    if (stressLinks.length > 0) ennInsights.push({ icon: '\u2198', label: 'Stress Arrow Web', desc: `Stress arrow connections exist between members: ${stressLinks.join(', ')}. Under pressure, one person's regression may trigger another's.`, color: '#e88050' });

    const growthLinks = [];
    ep.forEach((pA, i) => ep.forEach((pB, j) => { if (i !== j && ENN_ARROWS[pA.ennType].growth === pB.ennType) growthLinks.push(`${pA.label} \u2192 ${pB.label}`); }));
    if (growthLinks.length > 0) ennInsights.push({ icon: '\u2197', label: 'Growth Arrow Web', desc: `Growth connections between members: ${growthLinks.join(', ')}. At their best, these members naturally model each other's growth direction.`, color: '#50c878' });

    if (ep.every(p => p.ennType === ep[0].ennType)) ennInsights.push({ icon: '\u2726', label: `All Type ${ep[0].ennType} \u2014 Mirror Group`, desc: `Every member shares the same core type. Instant mutual legibility \u2014 but collective blind spots and defense mechanisms are amplified with no internal counterbalance.`, color: G.gold });
  }

  if (persons.every(p => p.instinctStack?.length > 0) && persons.length >= 2) {
    const ic = { sp: 0, sx: 0, so: 0 };
    persons.forEach(p => { const dom = p.instinctStack[0]; if (dom) ic[dom] = (ic[dom] || 0) + 1; });
    ...
    if (activeI.length === 1) {
      ...
      instinctInsights.push({ icon: '◆', label: `All ${inst.toUpperCase()} Instinct`, desc: `Every member shares the ${inst.toUpperCase()} instinctual variant, all focused on ${iDesc}. Deep shared priorities \u2014 but the group may be blind to what the other instincts provide.`, color: '#e88050' });
    }
  }
  ...
  if (mp.length >= 2) {
    ...
    if (ei.E === 0) mbtiInsights.push({ icon: '\u25b8', label: 'All Introverts', desc: 'No extraverts. Processing will be slow and deliberate \u2014 everyone prefers to think before speaking. Risk: ideas stay internal, decisions stall, nobody naturally drives external engagement.', color: '#4a88d8' });
    else if (ei.I === 0) mbtiInsights.push({ icon: '\u25b8', label: 'All Extraverts', desc: "No introverts. Energy will be high and dialogue fast-moving. Risk: insufficient reflection \u2014 the group may push decisions before they're fully considered.", color: '#4a88d8' });

    const sn = { S: 0, N: 0 };
    mp.forEach(p => sn[p.mbti[1]]++);
    if (sn.S === 0) mbtiInsights.push({ icon: '\u2666', label: 'No Sensing Types', desc: 'All intuitives. Excels at vision and pattern-finding but may chronically under-invest in practical execution and present-moment detail. Grounding abstract ideas may be a persistent challenge.', color: '#e8a030' });
    else if (sn.N === 0) mbtiInsights.push({ icon: '\u2666', label: 'No Intuitive Types', desc: 'All sensors. Strong on execution and practical detail \u2014 but may resist big-picture thinking and long-range possibility. Innovation and reframing may require deliberate effort.', color: '#e8a030' });

    const tf = { T: 0, F: 0 };
    mp.forEach(p => tf[p.mbti[2]]++);
    if (tf.T === 0) mbtiInsights.push({ icon: '\u2666', label: 'No Thinking Types', desc: 'All feeling types. Relational harmony dominates decision-making. Risk: logical inconsistencies may go unchallenged. An outside T-type perspective for critical decisions is worth seeking.', color: '#b850c0' });
    else if (tf.F === 0) mbtiInsights.push({ icon: '\u2666', label: 'No Feeling Types', desc: 'All thinking types. Logic and efficiency dominate. Risk: emotional impact and morale may be systematically underweighted until they become unavoidable problems.', color: '#b850c0' });

    const jp = { J: 0, P: 0 };
    mp.forEach(p => jp[p.mbti[3]]++);
    if (jp.J === 0) mbtiInsights.push({ icon: '\u2666', label: 'All Perceivers', desc: 'No judgers. Flexibility is a strength but structure and closure may be chronically avoided. The group can generate enormous exploration without converging \u2014 external accountability mechanisms are likely needed.', color: '#30a888' });
    else if (jp.P === 0) mbtiInsights.push({ icon: '\u2666', label: 'All Judgers', desc: 'No perceivers. Organization and follow-through are strengths, but the group may close on decisions too quickly and resist new information after a plan is set.', color: '#30a888' });

    const dc = {};
    mp.forEach(p => { const fn = MBTI_TYPES[p.mbti]?.stack[0]; if (fn) dc[fn] = (dc[fn] || 0) + 1; });
    const topFn = Object.entries(dc).sort((a, b) => b[1] - a[1])[0];
    if (topFn && topFn[1] > 1) mbtiInsights.push({ icon: '◆', label: `${topFn[1]}\u00d7 ${topFn[0]}-Dominant`, desc: `${topFn[1]} members lead with ${topFn[0]} (${COG_FUNCTIONS[topFn[0]]?.name}) \u2014 this function will heavily shape the group's default mode of engagement.`, color: G.gold });
    ...
    if (!hasF && hasT) mbtiInsights.push({ icon: '\u2197', label: 'No Feeling-Dominant Members', desc: "Nobody leads with a Feeling function. Interpersonal dynamics and values-based considerations will be handled reactively via auxiliary or lower functions rather than proactively.", color: '#b850c0' });
    if (!hasT && hasF) mbtiInsights.push({ icon: '\u2197', label: 'No Thinking-Dominant Members', desc: "Nobody leads with a Thinking function. Logical rigor and structural analysis are available but not primary \u2014 they'll require deliberate effort to bring forward.", color: '#4a88d8' });
  }

  return { ennInsights, mbtiInsights, instinctInsights };
}
